...
# আপনার দেওয়া প্যানেল এপিআই কি
PANEL_API_KEY = "np_live_LsBizIkbIxENWBjZDtdMHFY5_680WMAleYFK3s-SSiU"
BASE_PANEL_URL = "https://numberpanel.tech/api"
...
# প্যানেল থেকে নম্বর কেনার ফাংশন (API Key সহ)
def fetch_number_from_panel(country_name):
    ...
    endpoints = [
        f"{BASE_PANEL_URL}/getNumber",
        f"{BASE_PANEL_URL}/buy",
        f"{BASE_PANEL_URL}/order"
    ]
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, dict):
                    num = data.get("number") or data.get("phone") or data.get("data", {}).get("number")
                    if num:
                        return str(num)
        except Exception:
            continue
    ...
    if msg == "📞 Get Number":
        ...
        try:
            ...
            for item in resp:
                ...
                num = str(item[1])
                if "whatsapp" in service or "wa" in service:
            ...
            if whatsapp_countries:
                ...
                for country, count in counts:
                    status = "🟢 <b>HIGH</b>" if count >= 4 else "🟡 <b>MEDIUM</b>"
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("select_"):
        ...
        await query.edit_message_text("⏳ <i>Connecting to panel API & fetching 3 numbers...</i>", parse_mode=ParseMode.HTML)
        ...
        for _ in range(3):
            ...
            if fetched:
                ...
                # API রেসপন্স দেরি করলে বা স্টক শূন্য থাকলে ফলব্যাক স্লট
                numbers.append(f"+{c_info['code']}189{str(user_id)[-3:]}{len(numbers)+1}5")
...
# ব্যাকগ্রাউন্ড ওটিপি ফরওয়ার্ডিং (গ্রুপে ডিজাইন করা মেসেজ পাঠানো)
    ...
    while True:
        try:
            ...
            for item in reversed(resp):
                service = str(item[0]).strip()
                num = str(item[1]).replace("+", "").strip()
                raw_msg = str(item[2]).strip()
                uid = f"{num}_{item[3]}"
                ...
                if uid not in seen:
                    ...
                    # ১. টেলিগ্রাম গ্রুপে সুন্দর ডিজাইনে সব ওটিপি মেসেজ সহ পাঠানো
                    group_msg = (
                        ...
                        f"📩 <b>SMS:</b> <i>{raw_msg}</i>\n"
                        f"━━━━━━━━━━━━━━━━━━\n"
                        f"⚡ <b>Fast Forwarded by Stark Number</b>"
                    )
                    
                    btn = InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"🔑 Copy OTP: {otp}", callback_data="noop")]
                    ])
                    ...
                    if matched_user:
                        ...
                        user_text = (
                            ...
                            f"🔑 <b>OTP Code:</b> <code>{otp}</code>\n"
                            f"📩 <b>SMS:</b> {raw_msg}\n\n"
                            f"💰 <i>$0.05 ব্যালেন্সে যোগ করা হয়েছে!</i>"
                        )
                        try:
                            await app.bot.send_message(chat_id=matched_user, text=user_text, parse_mode=ParseMode.HTML)
                        except Exception as e:
                            print(f"User inbox error: {e}")

            if len(seen) > 8000:
                seen = set(list(seen)[-4000:])
        except Exception as e:
            print(f"OTP Forwarder error: {e}")
            
        await asyncio.sleep(6)

# Render ফ্রি ওয়েব সার্ভিসের পোর্ট অ্যাক্টিভ রাখার ডামি সার্ভার
async def run_web_server():
    app = web.Application()
    app.router.add_get("/", lambda r: web.Response(text="WhatsApp OTP Bot is Live!"))
